...
- 真实 pi 核心 probe。
- 112 项自动测试、类型检查、文档链接检查和 extension smoke。
...
P0 至 P4 的产品功能人工验收均已通过。P4 已证明 Markdown/txt 创建、确认启用、立即学习和放弃删除；当前进入 P5 Profile 修订闭环。
...
状态：代码、112 项自动测试、extension smoke 和真实九图 Agent probe 已通过；等待 1 个真实 Pi 人工验收轮次。
...
从当前状态估计，里程碑一还需要约 4～7 个有效开发轮次。

这个范围允许：

- P5 Profile 修订预留 1 轮真实 TUI 验收及必要修正。
...
1. 实现从 active 复制 revision draft、已有 draft 继续修订和自然语言修订意见输入。
2. 建立受影响文件计划、原子修改、结构校验和独立质量审查。
3. 提供继续修改、确认启用并归档旧 active、放弃 draft 三个出口。

P5 人工门禁通过前不启动 P6 长会话与故障稳定验收。
