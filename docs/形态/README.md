...
- 代码计算补丁相对当前 draft 的实际 before/after，用户二次确认后才写入；Agent 自报摘要不能代替 diff。
...
- 启用前比较 draft 与当前 active 的全部业务内容；仅 quality report/manifest 变化或连续修改互相抵消时禁止启用。
...
P5 已按产品功能口径关闭。A 复测确认代码计算的实际 diff、revision 启用和旧 active 归档正常；C 放弃出口正常。P6 已扩展 `/study-recover`，B 这类已有 attempt、无 summary 的 interrupted 批次现在可以被选择并补总结。
...
测试数据和原始用户侧记录只在本机私有保存，不进入开源仓库。汇总证据已经证明：
...
- 新增 `/study-recover`：可列出 active Profile 下遗留的 `running` 会话，以及有 attempt、无 summary 的 `interrupted` 会话；running 可补总结或标记中断，interrupted 只能补总结。
- 补总结失败不会改写原记录状态，session 保持原来的 `running` 或 `interrupted`，可以稍后重试。
- 没有题目记录的 running 会话可以直接标记中断，不需要模型。
- 没有题目记录的 interrupted 会话不进入恢复候选。
- Pi 启动时发现上述待处理会话会显示数量和 `/study-recover` 提示。
...
- interrupted 补总结入口已通过自动测试，等待真实 Pi 复核现有 B 批次。
- 10 题以上长会话、compaction、取消、超时和第二模型提供方验证。
- Agent Run、模型 turn 和底层 HTTP 请求的完整成本观测。
- clean clone、CI 配置、42 文件 npm pack、依赖审计和敏感数据门禁已完成；Git 历史清理与最终发布决定尚未完成。
