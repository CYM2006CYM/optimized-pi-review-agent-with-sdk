...
阶段 0、0.5、Profile family 基础和真实 SDK Agent 核心 probe 已完成。P0 第二轮人工测试 A、D 已通过；C 的放弃流程通过但 session mode 实际为 `card_practice`；B 的讨论未提交节点结果问题已增加一次自动重试和降级返回。当前等待 B 与 `chapter_study` mode 复测，通过后进入 mode、difficulty rubric、证据化总结和范围历史。
