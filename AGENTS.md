...
阶段 0、0.5、Profile family 基础、P0 至 P5 已完成。当前处于 P6 稳定验收：`/study-recover` 已支持为有 attempt、无 summary 的 interrupted 批次补总结，等待真实 Pi 复核；开源前非文档准备已建立门禁，Git 历史清理仍需用户决定方案。
...
```powershell
npm test                    # 在本仓库中：当前 117 项测试
npm run typecheck           # 在本仓库中：TypeScript 类型检查
npm run check:docs          # 只检查项目 Markdown，不扫描 node_modules
npm run check:release       # 检查 tracked 私有数据、常见密钥和发布清单
npm run check:history       # 开源前检查 Git 历史中的私有运行数据
npm run audit:prod          # 生产依赖漏洞门禁
npm run audit:all           # 包含开发工具的完整依赖漏洞门禁
npm run verify              # 不调用模型的完整 CI 验证
npm run verify:open-source  # 加入历史、audit 和 pack 的开源门禁
